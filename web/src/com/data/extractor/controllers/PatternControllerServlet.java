...
public class PatternControllerServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(!isMultipart){
            ...
            uploadStatus.setId(patternDataElement.getExtractedId());
            uploadStatus.setParent("test");

            List<UploadStatus> uploadStatusList =null;
            uploadStatusList = xFilesDAO.getRecord(uploadStatus.getId());

            /* Load the Template PDF in to pdf BOX and return the PDDoc to set pdf Properties*/
            PDDocument doc =PDDocument.load(uploadStatusList.get(0).getUploadedPdfFile());

            StringBuilder sb=pdftoText(uploadStatusList.get(0).getUploadedPdfFile());
